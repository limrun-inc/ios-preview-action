...
function buildComment(platform: string, sha: string, previewUrl: string): string {
  return [
    ...
    `|----------|--------|---------|`,
    `| ${platformLabel[platform] || platform} | \`${sha.slice(0, 7)}\` | [Open Simulator Preview →](${previewUrl}) |`,
    `| ${platformLabel[platform] || platform} | \`${sha.slice(0, 7)}\` | [Install to Device →](${previewUrl}) |`,
    "",
